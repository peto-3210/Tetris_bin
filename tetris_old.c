...
int score = 0;
char GameOn = TRUE;
uint32_t timer = 400000; // decrease this to make it faster
int decrease = 1000;
...
void PrintTable(){
	...
	for(i = 0; i < ROWS ;i++){
		for(j = 0; j < COLS ; j++){
            ...
            Result[(int)(i / sizeof(uint64_t))] = (Table[i][j] + Buffer[i][j]) ? 
                Result[(int)(i / sizeof(uint64_t))] | (1 << ((i % COLS) * COLS + j)) :
                Result[(int)(i / sizeof(uint64_t))] & ~(1 << ((i % COLS) * COLS + j));
		}
		printw("\n");
	}
    ...
    char buffer_r[129] = {0};
    //char buffer_c[9] = {0};
    itoa(Result[0], buffer_r, 2);
    itoa(Result[1], buffer_r + 64, 2);
    printw("Table: %s\n", buffer_r);
    
    /*for (int i = 0; i < ROWS; ++i){
        char buf[9] = {0};
        memcpy(buf, buffer_r + 8 * i, sizeof(char));
        printw("%s\n", buf);
    }*/
}
...
int hasToUpdate(){
	return ((uint32_t)(now.tv_sec*1000000 + now.tv_usec) -((uint32_t)before_now.tv_sec*1000000 + before_now.tv_usec)) > timer && false;
}
